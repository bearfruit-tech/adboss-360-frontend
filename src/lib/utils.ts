...
export function generateMarketingResearchReportHTML(
  businessName: string,
  report: {
  ...
  const componentStyles = `
    ...
    p { font-size: 12px; line-height: 1.6; color: #111827; white-space: pre-line; }
    ...
    .card { border: 1px solid #e5e7eb; border-radius: 6px; padding: 12px; background: #ffffff; }
    .muted { color: #6b7280; font-size: 12px; }
    ul { margin: 0; padding-left: 18px; }
    li { font-size: 12px; line-height: 1.6; }
    .by-method { display: flex; flex-direction: column; gap: 8px; }
    .by-method-item { border: 1px solid #e5e7eb; border-radius: 6px; padding: 10px; }
    .by-method-title { font-weight: 600; margin-bottom: 4px; font-size: 12px; }
    .footer { position: fixed; bottom: 12mm; left: 20mm; right: 20mm; color: #9ca3af; font-size: 10px; }
  `;
  ...
  const html = `
    ...
    <html>
      <head>
        <meta charset="utf-8" />
        <title>Marketing Research Report</title>
        <style>${componentStyles}</style>
      </head>
      <body>
        <div class="container">
          <div class="header">
            <h1>${safe(businessName || 'Marketing Research Report')}</h1>
            <div class="muted">Professional research report (PDF) ready for stakeholders</div>
          </div>

          <div class="section card">
            <h2>Executive Summary</h2>
            <p>${safe(report.executiveSummary)}</p>
          </div>

          <div class="section card">
            <h2>Research Objectives Recap</h2>
            <p>${safe(report.objectivesRecap)}</p>
          </div>

          <div class="section card">
            <h2>Methodology Summary</h2>
            <p>${safe(report.methodologySummary)}</p>
          </div>

          <div class="section card">
            <h2>Findings by Method</h2>
            <div class="by-method">
              ${(report.findingsByMethod || [])
                .map(
                  (f) => `
                  <div class="by-method-item">
                    <div class="by-method-title">${safe(f.method)}</div>
                    <p>${safe(f.findings)}</p>
                  </div>
                `
                )
          ...
          <div class="section card">
            <h2>Cross-Method Analysis</h2>
            <p>${safe(report.crossMethodAnalysis)}</p>
          </div>

          <div class="section card">
            <h2>Strategic Implications</h2>
            <p>${safe(report.strategicImplications)}</p>
          </div>
          ...
          <div class="muted">Deliverable: ${safe(report.deliverableDescription || 'Professional research report')}</div>
        </div>
        <div class="footer">Generated by AdBoss 360</div>
      </body>
    </html>
  `;
